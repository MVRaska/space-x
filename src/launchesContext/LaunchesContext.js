...
import {launchesReducer, initialState} from '../launchesReducer/launchesReducer';
...
const LaunchesContextProvider = ({children}) => {
  ...
  //     .catch((error) => {dispatch({type: "FETCH_LAUNCHES_FAILURE", payload: error.response})});
  // }, []);


    return <LaunchesContext.Provider value={{state, dispatch}}>{children}</LaunchesContext.Provider>
}
